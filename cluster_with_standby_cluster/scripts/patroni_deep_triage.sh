#!/bin/bash
# ============================================================================
# Patroni Deep Triage & Audit (patroni_deep_triage.sh)
# ============================================================================
#
# PURPOSE
#   Exhaustive, read-only diagnostics of a Patroni-managed PostgreSQL node
#   and its cluster. Flags issues, explains WHY they matter, and prints the
#   exact remediation commands — but never executes any change itself.
#
# WHY A DEDICATED SCRIPT
#   Patroni failures are rarely loud. The pathological states are the QUIET
#   ones: a paused cluster that looks green, a DCS lease about to expire, a
#   watchdog that silently failed to arm, a replica with nofailover that
#   everyone forgot about. This script hunts specifically for those.
#
# HOW PATRONI WORKS (the 30-second model that drives every check below)
#   Patroni is a supervisor process around PostgreSQL. Every loop_wait
#   seconds it runs its HA loop:
#     1. Read cluster state from the DCS (etcd/consul/zookeeper/k8s).
#     2. If I hold the leader key: refresh its TTL lease. If I cannot
#        refresh it within retry_timeout, I MUST demote myself (fencing) —
#        because another node may already be promoting.
#     3. If nobody holds the leader key: participate in the leader race
#        (healthiest eligible replica wins).
#     4. Converge local PostgreSQL config/role toward the DCS-declared state.
#   Consequences:
#     - DCS availability IS cluster availability for writes. No DCS quorum
#       -> leader self-demotes -> read-only cluster. (This is by design.)
#     - The timing rule ttl >= loop_wait + 2*retry_timeout exists so one
#       missed DCS round-trip doesn't expire the lease. Violating it causes
#       spurious demotions under normal network jitter.
#     - "paused" mode disables the whole loop: no failover will EVER happen
#       until resumed. Looks healthy in monitoring. Classic planted trap.
#
# USAGE
#   ./patroni_deep_triage.sh [--url http://127.0.0.1:8008] [--config <path>]
#   Runs best with sudo (config + journal access). Degrades gracefully.
#
# EXIT CODES
#   0 = ran to completion (issues may still be flagged in output)
#   1 = Patroni not present at all on this host
#
# COMPATIBILITY
#   Debian/Ubuntu + RHEL/Rocky/Alma. Pure bash + coreutils; uses curl or
#   wget or raw /dev/tcp for HTTP; python3/jq used only if present (output
#   pretty-printing), never required.
# ============================================================================
...
PATRONI_CONTAINER="${PATRONI_CONTAINER:-}"
CONTAINER_MODE=0
C_STATUS=""
STACK_CONTAINERS=""
if [ -n "$DOCKER" ]; then
...
# READ-ONLY cluster. That is a feature (fencing), not a bug — but on the
# challenge box it will look like "database suddenly rejects writes".
# ============================================================================
echo
...
if [ -n "$CFG" ]; then
    ...
    while read -r hp; do
        DCS_HOSTS+=("$hp")
    done < <(echo "$CFG" | awk '/^[a-zA-Z0-9_-]+:/ { if ($1 ~ /^(etcd3?|consul|zookeeper):/) in_dcs=1; else in_dcs=0 } in_dcs { print }' | \
...
if [ ${#DCS_HOSTS[@]} -gt 0 ]; then
    ...
    for hp in "${DCS_HOSTS[@]}"; do
        ...
        CONN_OK=0
        if [ $CONTAINER_MODE = 1 ]; then
            if cexec "timeout 1 bash -c 'echo > /dev/tcp/$H/$P'" >/dev/null 2>&1; then
                CONN_OK=1
            fi
        else
